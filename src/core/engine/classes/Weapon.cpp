...
bool Weapon::Update() {
    ...
    uintptr_t bucket_ptr = p->read<uintptr_t>(
        entity_list + offsets::EntityListOffset + offsets::ChunkStride * ((slot_index & offsets::IndexMask) >> offsets::HandleBits));
    if (!bucket_ptr)
        return false;

    uintptr_t weapon_ptr = p->read<uintptr_t>(bucket_ptr + offsets::EntryStride * (slot_index & offsets::HandleMask));
	if (!weapon_ptr)
		return false;
