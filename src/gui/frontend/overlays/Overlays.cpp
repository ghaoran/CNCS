...
bool Overlays::InitImpl() {
	...
	this->font_alt = io.Fonts->AddFontFromFileTTF(GetCJKFontPath(), 14.0f, &cfg);
    ...
    // Pre allocate buffer（至少 2 个采样点，避免除零）
    this->vel_buffer.resize(std::max<size_t>(
        static_cast<size_t>(cfg::world::velocity::sample_rate * cfg::world::velocity::sample_length), 2));

    return true;
}

void Overlays::RenderImpl() {
    // Fetch the snapshot ONCE (just a refcount bump now) and share it across
    // all sub-renders.
...
void Overlays::RenderWatermark(const Snapshot& snapshot) {
    ...
    std::string watermark_string = "Hello, CNCS!";
    ...
    auto pos = ImVec2(rect_start.x + padding, rect_start.y + padding * 0.6f/* compensate font */);
...
void Overlays::RenderSpectatorList(const Snapshot& snapshot) {
    ...
    if (!ImGui::Begin("观战列表", nullptr, flags)) {
        ImGui::End();
        return;
    }
    ...
    if (detailed) {
        if (ImGui::BeginTable("##detailed", 3, flags_table)) {
            ...
            for (const Player& player : players) {
                ...
                if (self_only) ImGui::Text("你");
                else if (player.observer_services.mode == ObserverMode::Free) ImGui::Text("无");
                else ImGui::Text("%s", target ? target->name : "无效/炸弹");
            }
...
void Overlays::RenderSpeedChart(const Snapshot& snapshot) {
    ...
    int rate = std::max(cfg::world::velocity::sample_rate, 1);
    float length = std::max(cfg::world::velocity::sample_length, 0.1f);
    ...
    if (is_menu_open) {
        const float height_padding = 25.f;  // some padding to keep the speed number inside the area
        const float altitude_padding = 10.f; // so it doesnt go under the titlebar

        ImGui::SetNextWindowBgAlpha(0.1f);
        ImGui::SetNextWindowPos(pos - Vec2_t(0.f, altitude_padding), ImGuiCond_Once);
        ImGui::SetNextWindowSize(size + Vec2_t(0.f, height_padding), ImGuiCond_Once);
        if (ImGui::Begin("速度图表", nullptr, ImGuiWindowFlags_NoCollapse))
        {
            pos = ImGui::GetWindowPos() + ImVec2(0.f, altitude_padding);
            size = ImGui::GetWindowSize() - ImVec2(0.f, height_padding);
            ImGui::End();
        }
    }
    ...
    if (prev_rate != rate || prev_length != length) {
        ...
        // 至少保留 2 个采样点，避免下方 buff_size-1 与 % buff_size 除零。
        vel_buffer.resize(std::max<size_t>(static_cast<size_t>(rate * length), 2));
    }

    Vec2_t speed_2d(local.vel.x, local.vel.y);
    int speed = static_cast<int>(speed_2d.len());
...
void Overlays::RenderDebugWindow(const Snapshot& snapshot) {
    ...
    constexpr int margin = 10;
    constexpr int padding = 10;
    std::string debug_string = "> 游戏调试窗口\n";

    debug_string += std::format("地图: {}\n", globals.map_name);
    debug_string += std::format("最大玩家数: {}\n", globals.max_clients);
    debug_string += std::format("缓存刷新: {}ms\n", cfg::dev::cache_refresh_rate);

    if (bomb.is_planted) {
        debug_string += "炸弹:\n";
        debug_string += std::format("- 安放点: {}\n", bomb.site == BombSite::A ? "A" : "B");
    }

    if (!players.empty())
        debug_string += std::format("玩家 ({}):\n", players.size());
...
void Overlays::RenderRadar(const Snapshot& snapshot) {
    ...
    if (is_menu_open) {
        ...
        if (ImGui::Begin("雷达", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar)) {
    ...
    d->AddText(ImVec2(pos.x + 6.f, pos.y + 4.f), IM_COL32(180, 180, 180, 200), "雷达");
}
...
void Overlays::RenderBomb(const Snapshot& snapshot) {
    ...
    if (is_menu_open) {
        ...
        if (ImGui::Begin("炸弹窗口", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize)) {
