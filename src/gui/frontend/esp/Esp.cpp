...
bool Esp::InitImpl() {
	...
	this->font = io.Fonts->AddFontFromFileTTF(GetCJKFontPath(), 12.0f, &cfg);
...
void Esp::RenderImpl() {
	...
	for (auto& player : players) {
		...
		bool mate = !cfg::deathmatch && player.team == local.team;

		if (!cfg::deathmatch && !cfg::esp::team && mate)
			continue;

		if (cfg::esp::spotted && !player.spotted)
			continue;

		// Cover check: only render players visible to us.
		if (cfg::esp::visible_only && !player.visible)
			continue;

		// Are we spectating the player in first person? then dont render
		// 修复：target 为 int，可能为 -1（无效），需先检查有效性
		if (
			local.observer_services.target > 0
			&& static_cast<uintptr_t>(local.observer_services.target) == player.pawn_controller_addr
...
void Esp::RenderPlayer(Player player, bool mate) {
	...
	Vec2_t head_screen;
	if (!player.GetBounds(matrix, io.DisplaySize, bounds, &head_screen))
		return;
	...
	// Precompute the team/enemy palette once per player instead of re-selecting
	// it inside every sub-renderer.
	const color_t box_color = mate ? cfg::esp::colors::box_team : cfg::esp::colors::box_enemy;
	const color_t skeleton_color = mate ? cfg::esp::colors::skeleton_team : cfg::esp::colors::skeleton_enemy;
	const color_t tracker_color = mate ? cfg::esp::colors::tracker_team : cfg::esp::colors::tracker_enemy;
	...
	if (cfg::esp::skeleton)
		RenderPlayerBones(player, skeleton_color);

	if (cfg::esp::head_tracker && player.skeleton_valid)
		RenderPlayerTracker(bounds, head_screen, tracker_color);

	RenderPlayerBars(player, bounds);
	RenderPlayerFlags(player, bounds, mate);
}

void Esp::RenderPlayerBones(Player player, color_t color) {
	if (!player.skeleton_valid)
		return;

	// Project every bone once (the 17 connections share endpoints, so this
	// avoids re-projecting shared joints like the neck/chest).
	std::array<Vec2_t, 30> pts;
	std::array<bool, 30> ok;
	for (int i = 0; i < 30; i++)
		ok[i] = matrix.wts(player.bone_list[i].pos, io.DisplaySize, pts[i]);

	for (const auto& bone : connections) {
		if (!ok[bone[0]] || !ok[bone[1]])
			continue;
		d->AddLine(pts[bone[0]], pts[bone[1]], ImColor(color), 1.5f);
	}
}

void Esp::RenderPlayerTracker(std::pair<Vec2_t, Vec2_t> bounds, Vec2_t head, color_t color) {
...
void Esp::RenderPlayerBars(Player player, std::pair<Vec2_t, Vec2_t> bounds) {
	if (cfg::esp::health) {
		...
		float filled_height = height * (std::clamp(player.health, 0, 100) / 100.0f);
		...
		if (cfg::esp::health_number && player.health < 100) {
			char hbuf[8];
			snprintf(hbuf, sizeof(hbuf), "%d", player.health);
			auto sz = ImGui::CalcTextSize(hbuf);
	...
	if (cfg::esp::armor) {
		...
		float filled_width = width * (std::clamp(player.armor, 0, 100) / 100.0f);
...
void Esp::RenderPlayerFlags(Player player, std::pair<Vec2_t, Vec2_t> bounds, bool mate) {
	if (cfg::esp::flags::name) {
		char name_buf[64];
		snprintf(name_buf, sizeof(name_buf), "%s%s", player.name, player.bot ? " (人机)" : "");
		auto name_size = ImGui::CalcTextSize(name_buf);
	...
	if (cfg::esp::flags::ammo && player.ammo != -1) {
		char abuf[8];
		snprintf(abuf, sizeof(abuf), "%d", player.ammo);
		auto ammo_size = ImGui::CalcTextSize(abuf);
	...
	float offset = 0.f;
	constexpr float offset_mult = 15.f;

	if (cfg::esp::flags::money && player.money) {
		char mbuf[16];
		snprintf(mbuf, sizeof(mbuf), "%d$", player.money);
		d->AddText(
	...
	if (cfg::esp::flags::ping && player.ping) {
		char pbuf[16];
		snprintf(pbuf, sizeof(pbuf), "%dms", player.ping);
		d->AddText(
			bounds.first - Vec2_t((bounds.first.x - bounds.second.x) - 10, offset),
			IM_COL32(255, 255, 255, 255),
			pbuf
